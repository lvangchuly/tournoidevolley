...
const upcomingMatches = useMemo(() => sortPublicMatchesByPriority(
  ...
  useEffect(() => {
    const live = getLiveRefereeMatch(selectedRefereeMatch) || selectedRefereeMatch;
    if (!live?.id || live.status === 'Valide') return;
    if (live.status !== 'Match en cours' && !live.refereeInProgress && !live.matchInProgress) return;
    const nextA = selectedRefereeMatch?.scoreA ?? live.scoreA ?? '';
    const nextB = selectedRefereeMatch?.scoreB ?? live.scoreB ?? '';
    if (String(nextA ?? '') === String(live.scoreA ?? '') && String(nextB ?? '') === String(live.scoreB ?? '') && String(live.submittedScoreA ?? '') === String(nextA ?? '') && String(live.submittedScoreB ?? '') === String(nextB ?? '')) return;
    syncRefereeLiveScore(live.id, nextA, nextB);
  }, [selectedRefereeMatch?.id, selectedRefereeMatch?.scoreA, selectedRefereeMatch?.scoreB]);
