...
export default function App() {
  ...
  useEffect(() => {
    if (isSmallTournamentMode) {
      ...
      if (activeTeamCount === 9 && stageValidation.championnatRetourComplete && singleKnockoutRef.current.quarters.length === 0) {
        ...
        if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
          const generated = generateNineTeamFinalsFromChampionshipReturn({ silent: true });
          if (generated) {
            autoGeneratedStageSignaturesRef.current.add(signature);
            return;
          }
      ...
      if (forceNineTeamFinalsIfReady({ silent: true })) {
        return;
      }
    ...
    if (!isSmallTournamentMode && !shouldSkipBrassage2 && stageValidation.brassage2Complete && !hasGeneratedMainStructure) {
  ...
  function randomizeNineTeamTournamentScores() {
    ...
    const leg2Complete = (championshipLeg2Ref.current?.matches || []).length > 0
      && (championshipLeg2Ref.current?.matches || []).every((match) => getMatchStatusLabel(match, rules) === 'Valide');
    if (leg2Complete && !(singleKnockoutRef.current?.quarters || []).length) {
  ...
  function randomizeCurrentPhaseScores() {
    try { if (typeof randomizeNineTeamTournamentScores === 'function' && randomizeNineTeamTournamentScores()) return; } catch (error) { console.error('Erreur score aléatoire 9 équipes', error); }
  ...
  function forceNineTeamFinalsIfReady(options = {}) {
    ...
    const retourMatches = Array.isArray(leg2.matches) ? leg2.matches : [];
    if (retourMatches.length === 0) return false;

    const retourComplete = retourMatches.every((match) => getMatchStatusLabel(match, phaseRulesRef.current) === 'Valide');
    if (!retourComplete) {
    ...
    const teamIds = (
      leg2.pools?.[0]?.teamIds
      || leg1.pools?.[0]?.teamIds
    ...
    const rankedIds = computeRanking(
      teamIds,
      [
        ...(Array.isArray(leg1.matches) ? leg1.matches : []),
        ...retourMatches,
      ],
      teamMap,
      phaseRulesRef.current,
    ...
    if (rankedIds.length < 8) {
      if (!options?.silent && typeof window !== 'undefined') {
        window.alert('Impossible de générer les quarts : il faut 8 équipes classées après le Championnat Retour.');
      }
      return false;
    }

    const startSlot = stageSlotCount(Array.isArray(leg1.matches) ? leg1.matches.length : 0)
      + stageSlotCount(retourMatches.length);
  ...
  function generateNineTeamFinalsFromChampionshipReturn(options = {}) {
    return forceNineTeamFinalsIfReady(options);
  }
  ...
  function approveRefereeScore(scope, matchId) {
    ...
    queueMicrotask(() => {
      ...
      if (forceNineTeamFinalsIfReady({ silent: true })) return;
      if (stableGenerateAfterBrassage2({ silent: true })) return;
      if (forceNineTeamFinalsIfReady({ silent: true })) return;
      if (generateNineTeamFinalsFromChampionshipReturn({ silent: true })) return;
      if (generateNineTeamFinalsFromChampionshipReturn({ silent: true })) return;
