...
const STORAGE_KEY = 'tournoidevolley-react-vite-V27BF';
...
export default function App() {
  ...
  const loadHomeAnnouncement = useCallback(async () => {
    ...
    try {
      const response = await fetch(buildFirebaseHomeAnnouncementUrl(), { cache: 'no-store' });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const payload = await response.json();
      const nextText = typeof payload === 'string'
        ? payload
  ...
  const saveHomeAnnouncement = useCallback(async (nextValue) => {
    if (!FIREBASE_DATABASE_URL) {
      setHomeAnnouncementEditing(false);
      setHomeAnnouncementText(String(nextValue || ''));
      setHomeAnnouncementDraft(String(nextValue || ''));
      return;
    }
    ...
    try {
      ...
      const response = await fetch(buildFirebaseHomeAnnouncementUrl(), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: normalizedValue, updatedAt: new Date().toISOString() }),
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
