...
function buildSmallSemisFromRanking(rankedIds) {
  const ids = Array.isArray(rankedIds) ? rankedIds.filter(Boolean) : [];
  return [
    makeKnockoutMatch('Tableau principal', 'Demi 1', ids[0] || null, ids[3] || null),
    makeKnockoutMatch('Tableau principal', 'Demi 2', ids[1] || null, ids[2] || null),
  ].filter((match) => match.teamAId && match.teamBId);
}

function buildSmallFinalsFromRanking(rankedIds) {
  const ids = Array.isArray(rankedIds) ? rankedIds.filter(Boolean) : [];
  return [
    makeKnockoutMatch('Tableau principal', 'Finale', ids[0] || null, ids[1] || null),
  ].filter((match) => match.teamAId && match.teamBId);
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (activeTeamCount >= 13 && !isSmallTournamentMode && !shouldSkipBrassage2 && stageValidation.brassage2Complete && !hasGeneratedMainStructure) {
  ...
  function generateDirectPrincipalQuartersForSmallTournament(options = {}) {
    const activeTeams = teamsRef.current.filter((team) => team.name.trim());
    const teamCount = activeTeams.length;

    // 8 et 9 utilisent déjà la méthode championnat dédiée de V33Q.
    if (teamCount >= 13 || teamCount === 8 || teamCount === 9) return false;
    ...
    const hasMainStage = (mainStageRef.current?.principaleMatches?.length || 0) > 0
      ...
      || (mainStageRef.current?.principalePools?.length || 0) > 0
      || (mainStageRef.current?.consolantePools?.length || 0) > 0;
    const hasKnockout = (knockoutRef.current?.principalQuarters?.length || 0) > 0
      || (knockoutRef.current?.principalSemis?.length || 0) > 0
      || (knockoutRef.current?.principalFinals?.length || 0) > 0;

    if (hasMainStage || hasKnockout) return false;
    ...
    const nextKnockout = {
      ...
      principalQuarters: rankedIds.length >= 8
        ? sanitizeKnockoutMatches(stampGeneratedMatches(assignScheduleWithCourts(buildQuarterMatchesFromRanking(rankedIds), startSlot, courts)))
        : [],
      principalSemis: rankedIds.length < 8 && rankedIds.length >= 4
        ? sanitizeKnockoutMatches(stampGeneratedMatches(assignScheduleWithCourts(buildSmallSemisFromRanking(rankedIds), startSlot, courts)))
        : [],
      principalFinals: rankedIds.length < 4
        ? sanitizeKnockoutMatches(stampGeneratedMatches(assignScheduleWithCourts(buildSmallFinalsFromRanking(rankedIds), startSlot, courts)))
        : [],
      principalEighths: [],
      consolanteEighths: [],
      consolanteQuarters: [],
    ...
    markPendingStructureSync();
    queueBackgroundCloudSave(250);
    triggerAutomaticBackup({ phaseName: 'Phase finale principale', mainStage: emptyMainStage, knockout: nextKnockout });
    return true;
  }
  ...
  function approveRefereeScore(scope, matchId) {
    ...
    queueMicrotask(() => {
      ...
      if (generateDirectPrincipalQuartersForSmallTournament({ silent: true })) return;
      if (forceSmallChampionshipQuartersIfReady({ silent: true })) return;
      if (forceSmallChampionshipQuartersIfReady({ silent: true })) return;
