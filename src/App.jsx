...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (safeGenerateMainStageAfterBrassage2()) {
      return;
    }
  ...
  function forceNineTeamFinalsIfReady(options = {}) {
    ...
    if (!retourComplete) {
      if (!options?.silent) window.alert('Tous les scores du Championnat Retour doivent être valides avant de générer les quarts de finale.');
      return false;
    }
  ...
  function approveRefereeScore(scope, matchId) {
    ...
    const safeGenerateMainStageAfterBrassage2 = () => {
      try {
        const activeCount = teamsRef.current.filter((team) => team.name.trim()).length;
        if (activeCount >= 3 && activeCount <= 9) return false;

        const b2Matches = brassage2Ref.current?.matches || [];
        const b2Complete = b2Matches.length > 0
          && b2Matches.every((match) => getMatchStatusLabel(match, phaseRulesRef.current) === 'Valide');

        const hasMainStage = (mainStageRef.current?.principaleMatches?.length || 0) > 0
          || (mainStageRef.current?.consolanteMatches?.length || 0) > 0
          || (mainStageRef.current?.principalePools?.length || 0) > 0
          || (mainStageRef.current?.consolantePools?.length || 0) > 0
          || (knockoutRef.current?.principalEighths?.length || 0) > 0
          || (knockoutRef.current?.principalQuarters?.length || 0) > 0
          || (knockoutRef.current?.principalSemis?.length || 0) > 0
          || (knockoutRef.current?.principalFinals?.length || 0) > 0
          || (knockoutRef.current?.consolanteEighths?.length || 0) > 0
          || (knockoutRef.current?.consolanteQuarters?.length || 0) > 0
          || (knockoutRef.current?.consolanteSemis?.length || 0) > 0
          || (knockoutRef.current?.consolanteFinals?.length || 0) > 0;

        if (!b2Complete || hasMainStage) return false;

        return generateMainStage(false) !== false;
      } catch (error) {
        console.error('Erreur sécurisée lors du passage Brassage 2 → phase suivante', error);
        if (typeof window !== 'undefined') {
          window.alert('Erreur lors de la génération de la phase suivante. Le tournoi est conservé ; vérifie les équipes et relance la génération.');
        }
        return false;
      }
    };
    ...
    queueMicrotask(() => {
      ...
      if (safeGenerateMainStageAfterBrassage2()) return;
