...
export default function App() {
  ...
  function generateNineTeamFinalsFromChampionshipReturn(options = {}) {
    ...
    const rankedIds = computeRanking(poolTeamIds, allChampionshipMatches, currentTeamMap, phaseRulesRef.current, { normalizeByMatches: false })
      .slice(0, 8)
      .map((row) => row.teamId)
      .filter(Boolean);

    if (rankedIds.length < 8) {
      if (!options?.silent) window.alert('Le classement du Championnat Retour ne contient pas encore 8 équipes qualifiées.');
      return false;
    }
    ...
    const nextSingleKnockout = { ...currentSingleKnockout, quarters, semis: [], finals: [] };
    singleKnockoutRef.current = nextSingleKnockout;
    setSingleKnockout(nextSingleKnockout);
    setActiveTab('finales');
    markPendingStructureSync();
