...
export default function App() {
  ...
  const hideSaveModeAleatButton = useMemo(() => !isSmallTournamentMode && hasAnyValidMatch(brassage1.matches), [isSmallTournamentMode, brassage1.matches, phaseRules]);
    ...
    <div className="app-shell">
      <div className={`container ${['brassage1', 'brassage2', 'principale', 'consolante', 'finales'].includes(activeTab) ? 'organizer-phase-fullwidth' : ''}`.trim()}>
        ...
        <div className="stack-gap">
          ...
          {activeTab === 'export' && (
            <Section title="Sauvegarde" subtitle="Export, import et sauvegarde locale du tournoi." right={<><Button onClick={exportState}>Exporter JSON</Button>{!hideSaveModeAleatButton && <Button variant="secondary" onClick={randomizeTeamsAndLevels}>ALEAT</Button>}<Button variant="secondary" onClick={randomizeCurrentPhaseScores}>Score aléatoire</Button><Button variant="secondary" onClick={() => importRef.current?.click()}>Importer JSON</Button></>}>
