...
function buildPoolMatchDescriptors(pools, phase, preferredCourts) {
  return (Array.isArray(pools) ? pools : [])
    .filter((pool) => pool && Array.isArray(pool.teamIds) && pool.teamIds.filter(Boolean).length >= 2)
    .map((pool) => ({
      pool,
      phase,
      preferredCourts: Array.isArray(preferredCourts) && preferredCourts.length ? preferredCourts : [1, 2, 3],
...
function scheduleBrassageMatches(pools, phase, startSlot) {
  ...
  poolPairs.forEach(([firstPoolIndex, secondPoolIndex], courtIndex) => {
    ...
    [pools[firstPoolIndex], pools[secondPoolIndex]].filter(Boolean).forEach((pool) => {
      descriptors.push(...buildPoolMatchDescriptors([pool], phase, [court]));
    });
  });
...
export default function App() {
  ...
  const showPublicPrincipalePoolRanking = principalePublicRanking.length > 0;
  const showPublicConsolantePoolRanking = consolantePublicRanking.length > 0;
  ...
  function renderCompactBrassageBoard(pools, matches, standings, overallRows, scope) {
    ...
    return (
      <div className="compact-brassage-layout">
            ...
            return (
              <div key={pool.id} className="mini-card compact-brassage-pool">
                ...
                <div className="compact-brassage-columns">
                  ...
                  <div className="compact-brassage-column compact-brassage-column-center">
                    <div className="compact-brassage-title">Matchs</div>
                    <div className="compact-mini-table">
                      <div className="compact-mini-table-head compact-match-grid">
                        <span>Rot.</span>
                        <span>Match</span>
                        <span>Arbitre</span>
                        <span>Terrain</span>
                      ...
                      <div className="compact-mini-table-body">
                        {poolMatches.map((match) => {
                          ...
                          const badgeClass = status === 'Valide' ? 'badge-success' : status === 'Score invalide' ? 'badge-danger' : 'badge-neutral';
                          return (
                            <div key={match.id} className="compact-mini-table-row compact-match-grid">
                              <span className="compact-cell-strong">{match.round || '—'}</span>
                              <span className="compact-match-label">
                                <span>{resolveTeam(match.teamAId).name}</span>
                                <strong>{(match.scoreA !== '' && match.scoreB !== '') ? `${match.scoreA} - ${match.scoreB}` : 'vs'}</strong>
                                <span>{resolveTeam(match.teamBId).name}</span>
                              </span>
                              <span>{refereeTeam ? refereeTeam.name : '—'}</span>
                              <span>T{match.court || '—'}</span>
                              <span><span className={`badge ${badgeClass}`}>{status}</span></span>
                            </div>
                          );
                        })}
        ...
        <aside className="mini-card compact-brassage-overall">
          <div className="compact-brassage-title">Classement général</div>
          <div className="compact-mini-table">
            <div className="compact-mini-table-head compact-ranking-grid">
              <span>#</span>
              <span>Équipe</span>
      ...
      <div className="container">
        ...
        <div className="stack-gap">
          ...
          {activeTab === 'brassage1' && !isSmallTournamentMode && (
            <>
              <Section title={`Brassage 1 : ${formatRemainingMatchesLabel(visibleBrassage1Matches, phaseRules)}`} subtitle="À gauche les équipes de chaque poule, puis les matchs avec l’équipe arbitre, le terrain et la rotation, puis le classement poule et enfin le classement général." right={<Button onClick={generateBrassage2}>Générer brassage 2</Button>}>
                {renderCompactBrassageBoard(brassage1.pools, visibleBrassage1Matches, brassage1Standings, rankingAfterBrassage1, 'brassage1')}
              </Section>
          ...
          {activeTab === 'brassage2' && !isSmallTournamentMode && (
            <>
              <Section title={`Brassage 2 : ${formatRemainingMatchesLabel(visibleBrassage2Matches, phaseRules)}`} subtitle="À gauche les équipes de chaque poule, puis les matchs avec l’équipe arbitre, le terrain et la rotation, puis le classement poule et enfin le classement général." right={<Button onClick={generateMainStage}>Générer principale / consolante</Button>}>
                {renderCompactBrassageBoard(brassage2.pools, visibleBrassage2Matches, brassage2Standings, rankingAfterBrassages, 'brassage2')}
              </Section>
