...
function getPreferredBrassagePoolCount(teamCount) {
  ...
  // Objectif : privilégier des poules de 3 équipes.
  // Quand Math.ceil(teamCount / 3) créerait une ou plusieurs poules de 2,
  // on supprime ces poules de 2 et on répartit leurs équipes dans les autres poules.
  // Le résultat ne contient donc que des poules de 3 ou de 4 équipes.
  return Math.floor(teamCount / 3);
}
...
function isMatchSelectableByReferee(match, phaseRules) {
  ...
  if (match.status === 'Résultats envoyés') return false;
...
function getDisplayScoreA(match) {
  return match?.scoreA !== '' && match?.scoreA !== null && match?.scoreA !== undefined ? match.scoreA : (match?.submittedScoreA ?? '');
}
...
function getMatchStatusLabel(match, phaseRules) {
  if (match?.status === 'Valide') return 'Valide';
...
const upcomingMatches = useMemo(() => {
  ...
  useEffect(() => {
    if (isSmallTournamentMode) {
      ...
      if (singleKnockoutRef.current.quarters.length > 0 && stageValidation.quarterComplete && singleKnockoutRef.current.semis.length === 0) {
        const signature = `auto-small-ko-2-${singleKnockoutRef.current.quarters.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`;
        if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
          autoGeneratedStageSignaturesRef.current.add(signature);
      ...
      if (singleKnockoutRef.current.semis.length > 0 && stageValidation.semiComplete && singleKnockoutRef.current.finals.length === 0) {
        const signature = `auto-small-ko-3-${singleKnockoutRef.current.semis.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`;
        if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
          autoGeneratedStageSignaturesRef.current.add(signature);
    ...
    if (stageValidation.brassage1Complete) {
      if (shouldSkipBrassage2) {
        ...
        if (!brassage2AlreadyGenerated) {
          ...
          const signature = `auto-brassage2-${validatedMatchCount}-${brassage1Ref.current.matches.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}:${match.validatedAt || ''}`).join('|')}`;
          if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
            try {
    ...
    if (!isSmallTournamentMode && teamsRef.current.filter((team) => team.name.trim()).length >= 13 && !shouldSkipBrassage2 && stageValidation.brassage2Complete && !hasGeneratedMainStructure) {
      const signature = `auto-main-stage-${(brassage2Ref.current?.matches || []).map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}:${match.validatedAt || ''}`).join('|')}`;
      if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
        try {
    ...
    if (stageValidation.consolantePoolsComplete && (knockoutRef.current.consolanteSemis || []).length === 0 && (knockoutRef.current.consolanteFinals || []).length === 0 && mainStageDistribution.consolanteMode !== 'direct-podium') {
      const signature = mainStageDistribution.consolanteMode === 'quarter-pools'
        ? `auto-consolante-quarts-${mainStage.consolanteMatches.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`
    ...
    if (knockoutRef.current.consolanteQuarters.length >= 4 && stageValidation.consolanteQuartersComplete && (knockoutRef.current.consolanteSemis || []).length === 0) {
      const signature = `auto-consolante-semis-${knockoutRef.current.consolanteQuarters.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`;
      if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
        autoGeneratedStageSignaturesRef.current.add(signature);
    ...
    if (knockoutRef.current.principalQuarters.length > 0 && stageValidation.principalQuartersComplete && (knockoutRef.current.principalSemis || []).length === 0) {
      const signature = `auto-principal-semis-${knockoutRef.current.principalQuarters.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`;
      if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
        autoGeneratedStageSignaturesRef.current.add(signature);
    ...
    if (knockoutRef.current.consolanteSemis.length > 0 && stageValidation.consolanteSemisComplete && (knockoutRef.current.consolanteFinals || []).length === 0) {
      const signature = `auto-consolante-finals-${knockoutRef.current.consolanteSemis.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`;
      if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
        autoGeneratedStageSignaturesRef.current.add(signature);
    ...
    if (knockoutRef.current.principalSemis.length > 0 && stageValidation.principalSemisComplete && knockoutRef.current.principalFinals.length === 0) {
      const signature = `auto-principal-finals-${knockoutRef.current.principalSemis.map((match) => `${match.id}:${getDisplayScoreA(match)}-${getDisplayScoreB(match)}`).join('|')}`;
      if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
        autoGeneratedStageSignaturesRef.current.add(signature);
  ...
  const filterRefereeVisibleMatches = useCallback((matches) => (
    dedupeMatches(Array.isArray(matches) ? matches : [])
      .filter((match) => {
        if (!hasBothTeamsDefined(match)) return false;
        const status = getMatchStatusLabel(match, phaseRules);
        if (status === 'Valide' || match?.status === 'Valide') return false;
        if (status === 'Résultats envoyés' || match?.status === 'Résultats envoyés' || Boolean(match?.pendingResultSentAt)) return false;
        return status === 'A saisir' || status === 'À saisir' || status === 'Match en cours';
      })
  ), [phaseRules]);
  ...
  const refereeSelectedEntry = useMemo(() => {
    ...
    // V36A stable : dès que l'organisateur valide un résultat envoyé,
    // le match doit disparaître de l'affichage Arbitres, y compris s'il était encore sélectionné.
    if (!match || getMatchStatusLabel(match, phaseRulesRef.current) === 'Valide') return null;
    return { ...group, match };
  ...
  function hasOfficialScorePairEmpty(match) {
    const scoreAEmpty = match?.scoreA === '' || match?.scoreA === null || match?.scoreA === undefined;
    const scoreBEmpty = match?.scoreB === '' || match?.scoreB === null || match?.scoreB === undefined;
    return scoreAEmpty && scoreBEmpty;
  }

  function isRandomScoreAvailableMatch(match) {
    if (!match) return false;
    const status = getMatchStatusLabel(match, phaseRulesRef.current);
    if (status === 'Valide' || status === 'Résultats envoyés') return false;
    if (match.refereeInProgress || match.matchInProgress || match.refereeStartedAt) return false;
    return hasOfficialScorePairEmpty(match);
  }

  function hasPlayablePendingMatches(matches) {
    return Array.isArray(matches)
      && matches.some((match) => isRandomScoreAvailableMatch(match));
  }

  function fillRandomScope(scope, matches) {
    if (!hasPlayablePendingMatches(matches)) return false;
    return applyRandomScores(scope);
  }
  ...
  function progressStructureForRandomScore() {
    ...
    try {
      ...
      // V42 : pour 36 équipes, Score aléatoire franchit aussi les huitièmes.
      // generatePrincipalSemis() génère les quarts quand les huitièmes sont validés et qu'aucun quart n'existe.
      if (currentTeamCount === 36 && eighthsComplete && principalQuarters.length === 0 && typeof generatePrincipalSemis === 'function') {
        generatePrincipalSemis();
        progressed = true;
      }
    ...
    try {
      ...
      // V42 : même correction pour la consolante 36 équipes : huitièmes -> quarts -> demies -> finales.
      if (currentTeamCount === 36 && eighthsComplete && consolanteQuarters.length === 0 && typeof generateConsolanteSemis === 'function') {
        generateConsolanteSemis();
        progressed = true;
      }
  ...
  const rules = phaseRulesRef?.current || phaseRules || null;
  const isWinning = isMatchResultValid(match, rules);

  if (isWinning) {
    return {
      ...match,
      status: 'Valide',
      validatedAt: match.validatedAt || Date.now(),
      refereeInProgress: false,
      matchInProgress: false,
      refereeStartedAt: null,
      submittedScoreA: '',
  ...
  // V37 : si l'organisateur modifie un match déjà Valide avec un score non gagnant,
  // le match redevient disponible pour les arbitres sans rester verrouillé par un arbitre.
  if (match.status === 'Valide') {
    return {
      ...match,
      status: 'Match en cours',
      validatedAt: null,
  ...
  function forceValidateOrganizerMatch(matchId) {
    updateMatchById(matchId, (match) => {
      const scoreA = match.scoreA !== '' && match.scoreA !== null && match.scoreA !== undefined
        ? match.scoreA
        : (match.submittedScoreA !== '' && match.submittedScoreA !== null && match.submittedScoreA !== undefined ? match.submittedScoreA : '');
      const scoreB = match.scoreB !== '' && match.scoreB !== null && match.scoreB !== undefined
        ? match.scoreB
        : (match.submittedScoreB !== '' && match.submittedScoreB !== null && match.submittedScoreB !== undefined ? match.submittedScoreB : '');
      return {
        ...match,
        scoreA,
        scoreB,
  ...
  function syncRefereeLiveScore(matchId, nextScoreA, nextScoreB) {
    updateMatchById(matchId, (match) => ({
      ...match,
      status: match.status === 'Valide' ? 'Valide' : 'Match en cours',
      scoreA: nextScoreA,
      scoreB: nextScoreB,
      submittedScoreA: nextScoreA,
      submittedScoreB: nextScoreB,
      submittedAt: Date.now(),
      refereeInProgress: match.status === 'Valide' ? false : true,
      matchInProgress: match.status === 'Valide' ? false : true,
  ...
  function handleRefereeScoreChange(field, value) {
    if (!selectedRefereeMatch) return;
    const liveMatch = getLiveRefereeMatch(selectedRefereeMatch) || selectedRefereeMatch;
    const normalizedValue = value === '' ? '' : String(value);
    const nextScoreA = field === 'scoreA' ? normalizedValue : (selectedRefereeMatch.scoreA ?? liveMatch.scoreA ?? '');
    const nextScoreB = field === 'scoreB' ? normalizedValue : (selectedRefereeMatch.scoreB ?? liveMatch.scoreB ?? '');
    const nextSelected = {
      ...selectedRefereeMatch,
      status: 'Match en cours',
      scoreA: nextScoreA,
      scoreB: nextScoreB,
      submittedScoreA: nextScoreA,
      ...
      refereeInProgress: true,
      matchInProgress: true,
      refereeStartedAt: selectedRefereeMatch.refereeStartedAt || liveMatch.refereeStartedAt || Date.now(),
    };
    setSelectedRefereeMatch(nextSelected);
    if (liveMatch?.id) syncRefereeLiveScore(liveMatch.id, nextScoreA, nextScoreB);
  }
  ...
  if (match?.status === 'Valide') return { text: 'Valide', className: 'success status-valide badge-success' };
  ...
  function approveRefereeScore(scope, matchId) {
    ...
    updateMatchesInScope(scope, (matches) => matches.map((match) => {
      ...
      if (isMatchResultValid(approved, phaseRulesRef.current)) {
        approved.status = 'Valide';
        approved.validatedAt = approvalTimestamp;
      } else {
        approved.status = 'Match en cours';
        approved.validatedAt = null;
      }
      return approved;
    }));
...
function applyRandomScores(scope) {
  ...
  updateMatchesInScope(targetScope, (matches) => (Array.isArray(matches) ? matches : []).map((match) => {
    // Le bouton Score aléatoire ne touche qu'aux matchs disponibles de la phase en cours :
    // pas encore validés, pas envoyés par arbitre, pas en cours d'arbitrage, et sans score officiel ScoreA/ScoreB.
    if (!isRandomScoreAvailableMatch(match)) return match;
                  ...
                  <tr key={match.id} className={status === 'Score invalide' ? 'row-invalid' : ''}>
                    ...
                    <td>
                      <div className="score-inputs">
                        <input type="number" min="0" inputMode="numeric" value={getDisplayScoreA(match)} onChange={(e) => updateOfficialMatchScore(scope, match.id, 'scoreA', e.target.value)} />
                        <span>-</span>
                        <input type="number" min="0" inputMode="numeric" value={getDisplayScoreB(match)} onChange={(e) => updateOfficialMatchScore(scope, match.id, 'scoreB', e.target.value)} />
                      </div>
                    </td>
                    ...
                    <div key={group.scope} className="mini-card public-ranking-card">
                      ...
                      <div className="referee-selector-list">
                        {group.matches.filter((match) => {
                          const status = getMatchStatusLabel(match, phaseRules);
                          return !match.pendingResultSentAt && status !== 'Valide' && match.status !== 'Valide' && status !== 'Résultats envoyés' && match.status !== 'Résultats envoyés';
                        }).map((match) => {
                          const schedule = scheduleData.scheduleMap[match.id];
                          const pendingStatus = getPendingStatus(match);
                          const officialStatus = getMatchStatusLabel(match, phaseRules);
