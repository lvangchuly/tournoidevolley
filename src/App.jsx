  ...
  function renderPodium(title, matches, fallbackPodium = null) {
    try {
      const fallbackRanking = fallbackPodium && typeof fallbackPodium === 'object'
        ...
        : [];
      const podium = extractPodiumFromStageMatches(matches, phaseRules, fallbackRanking);
      const firstTeamId = podium.first || null;
      const secondTeamId = podium.second || null;
      const thirdTeamId = podium.third || null;
      const safeTeam = (teamId) => {
        if (!teamId) return null;
        const team = resolveTeam(teamId);
        return {
          name: team?.name || 'À définir',
          level: normalizeLevelValue(team?.level, 'L'),
        };
      };
      const firstTeam = safeTeam(firstTeamId);
      const secondTeam = safeTeam(secondTeamId);
      const thirdTeam = safeTeam(thirdTeamId);
      return (
        <div className="mini-card public-ranking-card">
          ...
          <div className="podium-steps podium-steps-model">
            <div className="podium-lane podium-lane-second">
              <div className="podium-team-label">{secondTeam ? <TeamBadge name={secondTeam.name} level={secondTeam.level} className="podium-team-badge" /> : 'À venir'}</div>
            ...
            <div className="podium-lane podium-lane-first">
              <div className="podium-team-label">{firstTeam ? <TeamBadge name={firstTeam.name} level={firstTeam.level} className="podium-team-badge" /> : 'À venir'}</div>
              <div className="podium-players" aria-hidden="true"><img src={podiumPlayersHuman} alt="" className="podium-players-image" /></div>
            ...
            <div className="podium-lane podium-lane-third">
              <div className="podium-team-label">{thirdTeam ? <TeamBadge name={thirdTeam.name} level={thirdTeam.level} className="podium-team-badge" /> : 'À venir'}</div>
              <div className="podium-players" aria-hidden="true"><img src={podiumPlayersHuman} alt="" className="podium-players-image" /></div>
      ...
      console.error('Erreur affichage podium', error);
      return (
        <div className="mini-card public-ranking-card">
          <div className="mini-card-head">{title}</div>
          <div className="empty-state">Podium indisponible pour le moment.</div>
        </div>
      );
    }
  }
