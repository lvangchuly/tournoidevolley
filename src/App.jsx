...
function buildSaveModeFunctionnements() {
  return [
    ...
    "Pour moins de 13 équipes hors méthode championnat : après le Brassage 2, le classement cumulé Brassage 1 + Brassage 2 envoie directement les meilleures équipes en quarts de finale principale, sans création de poules principale/consolante. En dessous de 11 équipes, il n’y a pas de consolante.",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (buildDirectPrincipalQuartersAfterBrassage2({ silent: true })) {
      return;
    }

    if (!isSmallTournamentMode && activeTeamCount >= 13 && !shouldSkipBrassage2 && stageValidation.brassage2Complete && !hasGeneratedMainStructure) {
  ...
  function forceGenerateThirtySixFinalStagesAfterRandomScores() {
    ...
    let progressed = false;
      try { if (buildDirectPrincipalQuartersAfterBrassage2({ silent: true })) progressed = true; } catch {}
  ...
  function randomizeSmallChampionshipTournamentScores() {
    const currentTeamCount = teamsRef.current.filter((team) => team.name.trim()).length;
    if (currentTeamCount !== 8 && currentTeamCount !== 9) return false;
  ...
  function forceSmallChampionshipQuartersIfReady(options = {}) {
    ...
    if (rankedIds.length < 8) {
      if (!options?.silent && typeof window !== 'undefined') window.alert('Impossible de générer les quarts : il faut 8 équipes classées.');
      return false;
    }
  ...
  function buildDirectPrincipalQuartersAfterBrassage2(options = {}) {
    const activeTeams = teamsRef.current.filter((team) => team.name.trim());
    const teamCount = activeTeams.length;
    if (teamCount >= 13) return false;
    if (teamCount === 8 || teamCount === 9) return false;

    const b2Matches = brassage2Ref.current?.matches || [];
    const b1Matches = brassage1Ref.current?.matches || [];
    if (b2Matches.length === 0) return false;

    const b2Complete = b2Matches.every((match) => getMatchStatusLabel(match, phaseRulesRef.current) === 'Valide');
    if (!b2Complete) {
      if (!options?.silent && typeof window !== 'undefined') {
        window.alert('Tous les scores du Brassage 2 doivent être valides avant de générer les quarts de finale.');
      }
      return false;
    }

    const alreadyHasFinals = (knockoutRef.current?.principalQuarters || []).length > 0
      || (knockoutRef.current?.principalSemis || []).length > 0
      || (knockoutRef.current?.principalFinals || []).length > 0;
    const alreadyHasMain = (mainStageRef.current?.principaleMatches || []).length > 0
      || (mainStageRef.current?.principalePools || []).length > 0
      || (mainStageRef.current?.consolanteMatches || []).length > 0
      || (mainStageRef.current?.consolantePools || []).length > 0;
    if (alreadyHasFinals || alreadyHasMain) return false;

    const { teamMap } = buildCurrentTeamContext();
    const activeIds = activeTeams.map((team) => team.id).filter(Boolean);
    ...
    if (rankedIds.length < 2) return false;

    const quarterMatches = rankedIds.length >= 8
      ? buildQuarterMatchesFromRanking(rankedIds)
      : buildSeededKnockoutMatchesFromRanking(rankedIds, 'Tableau principal', rankedIds.length >= 4 ? 'Demi' : 'Finale');

    const startSlot = stageSlotCount(b1Matches.length) + stageSlotCount(b2Matches.length);
    const nextKnockout = {
      ...knockoutRef.current,
      principalEighths: [],
      principalQuarters: rankedIds.length >= 8
        ? sanitizeKnockoutMatches(stampGeneratedMatches(assignScheduleWithCourts(quarterMatches, startSlot, getCourtNumbers(CURRENT_COURT_COUNT))))
        : [],
      principalSemis: rankedIds.length < 8 && rankedIds.length >= 4
        ? sanitizeKnockoutMatches(stampGeneratedMatches(assignScheduleWithCourts(quarterMatches, startSlot, getCourtNumbers(CURRENT_COURT_COUNT))))
        : [],
      principalFinals: rankedIds.length < 4
        ? sanitizeKnockoutMatches(stampGeneratedMatches(assignScheduleWithCourts(quarterMatches, startSlot, getCourtNumbers(CURRENT_COURT_COUNT))))
        : [],
      consolanteEighths: [],
      consolanteQuarters: [],
    ...
    const emptyMainStage = { principalePools: [], principaleMatches: [], consolantePools: [], consolanteMatches: [] };
    knockoutRef.current = nextKnockout;
    mainStageRef.current = emptyMainStage;
    setKnockout(nextKnockout);
    setMainStage(emptyMainStage);
    setActiveTab('finales');
    markPendingStructureSync();
    queueBackgroundCloudSave(250);
    triggerAutomaticBackup({ phaseName: 'Quarts de finale principale', mainStage: emptyMainStage, knockout: nextKnockout });
    return true;
  }
  ...
  function approveRefereeScore(scope, matchId) {
    ...
    queueMicrotask(() => {
      ...
      if (buildDirectPrincipalQuartersAfterBrassage2({ silent: true })) return;
      if (forceSmallChampionshipQuartersIfReady({ silent: true })) return;
      if (forceSmallChampionshipQuartersIfReady({ silent: true })) return;
