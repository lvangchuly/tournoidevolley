    ...
    if (stageValidation.brassage1Complete) {
      if (shouldSkipBrassage2) {
        ...
        if (!brassage2AlreadyGenerated) {
          const signature = `auto-brassage2-${brassage1Ref.current.matches.map((match) => `${match.id}:${match.scoreA}-${match.scoreB}:${match.validatedAt || ''}`).join('|')}`;
          if (!autoGeneratedStageSignaturesRef.current.has(signature)) {
            try {
              const generated = generateBrassage2();
              if (generated !== false) {
                autoGeneratedStageSignaturesRef.current.add(signature);
                return;
              }
    ...
    queueMicrotask(() => {
      try {
        if (!isSmallTournamentMode && !shouldSkipBrassage2) {
          ...
          const brassage1Done = currentBrassage1Matches.length > 0 && currentBrassage1Matches.every((m) => getMatchStatusLabel(m, phaseRulesRef.current) === 'Valide');
          const brassage2Empty = (brassage2Ref.current?.matches || []).length === 0 && (brassage2Ref.current?.pools || []).length === 0;
          if (brassage1Done && brassage2Empty) {
            generateBrassage2();
          }
        }
      } catch (error) {
        console.error('Erreur auto Brassage 2 après validation', error);
      }
    });
  }
